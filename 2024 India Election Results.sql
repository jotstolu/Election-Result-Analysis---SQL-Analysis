...
select s.state,  
count(cr.parliament_constituency) as Total_seats_state 
...
-- or Total Seats by state
select s.state, count(parliament_constituency) as Total_seats_state
...
-- Single query to get all results 
...
select evm_votes, postal_votes, cd.candidate
...
--- ## alt method for the above query
...
JOIN 
    statewise_results sr ON cr.Parliament_Constituency = sr.Parliament_Constituency
JOIN states s ON sr.State_ID = s.State_ID
group by p.Party_alliance, s.state
...
-- or alt method for the above query
select top 10 cd.candidate, cr.constituency_name, cd.constituency_id, max(evm_votes) as EVM_votes
from constituencywise_details cd
JOIN 
    constituencywise_results cr ON cd.Constituency_ID = cr.Constituency_ID
group by cd.constituency_id, cd.candidate, cr.constituency_name
order by max(evm_votes) desc
